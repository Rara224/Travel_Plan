  ...
  tripPlan.value.days.forEach(day => {
    day.attractions.forEach(attraction => {
      const promise = apiClient
        .get('/api/poi/photo', { params: { name: attraction.name } })
        .then(res => res.data)
        .then((data: any) => {
...
const initMap = async () => {
  try {
    ...
    if (!amapKey) {
      message.warning('未配置高德地图 Key，地图功能已跳过')
      return
    }

    // 有些高德 Key 需要安全密钥（securityJsCode）才能正常加载
    const securityJsCode = (import.meta as any).env?.VITE_AMAP_SECURITY_JS_CODE as string | undefined
    if (securityJsCode && typeof window !== 'undefined') {
      ;(window as any)._AMapSecurityConfig = { securityJsCode }
    }

    // 等待容器渲染出有效尺寸，否则会出现“地图初始化了但画布不可见/空白”的情况
    await nextTick()
    const ok = await waitForMapContainer()
    if (!ok) {
      message.warning('地图容器尺寸异常，已跳过地图加载')
      return
    }
    ...
    map = new AMap.Map('amap-container', {
      ...
      // 部分手机浏览器/设备对 3D(WebGL) 支持不稳定，移动端默认 2D 更稳
    ...
    const msg = (error as any)?.message ? String((error as any).message) : '地图加载失败'
    message.error(msg)
  }
}
...
@media (max-width: 768px) {
  ...
  /* 强制景点列表在手机端单列 */
  .days-card :deep(.ant-list-grid .ant-col) {
    max-width: 100% !important;
    flex: 0 0 100% !important;
  }

  /* 天气列表在手机端单列 */
  #weather :deep(.ant-list-grid .ant-col) {
    max-width: 100% !important;
    flex: 0 0 100% !important;
  }

  /* 避免 iOS Safari 编辑输入聚焦时自动放大 */
  .result-container :deep(input),
  .result-container :deep(textarea),
  .result-container :deep(.ant-input-number-input) {
    font-size: 16px;
  }
}
</style>
