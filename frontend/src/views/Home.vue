    ...
    <a-card class="form-card" :bordered="false">
      ...
      >
        ...
        <div class="form-section">
          <div class="section-header">
            <span class="section-icon">📡</span>
            <span class="section-title">传感器输入(可选)</span>
          </div>

          <a-alert
            type="info"
            show-icon
            message="可选：授权定位信息(GPS)，用于生成从当前位置到目的地机场的出发交通建议。"
            class="sensor-alert"
          />

          <a-row :gutter="[16, 12]" class="sensor-actions">
            <a-col :xs="24" :sm="24" :md="24">
              <a-button block @click="captureLocation" :loading="sensorLoading.location">
                获取当前位置(GPS)
              </a-button>
              <div class="sensor-meta">
                <div class="sensor-status">状态：{{ sensorStatus.location }}</div>
                <div v-if="sensorContext.location" class="sensor-value">
                  经度 {{ sensorContext.location.longitude.toFixed(6) }}, 纬度 {{ sensorContext.location.latitude.toFixed(6) }}
                  <span v-if="sensorContext.location.accuracy_m"> (±{{ Math.round(sensorContext.location.accuracy_m) }}m)</span>
                </div>
              </div>
            </a-col>
          </a-row>

          <a-row v-if="sensorContext.captured_at" class="sensor-captured" :gutter="16">
            <a-col :span="24">
              <div class="sensor-time">最近采集时间：{{ sensorContext.captured_at }}</div>
            </a-col>
          </a-row>
        </div>
...
const sensorContext = reactive<SensorContext>({})
...
function touchCapturedAt() {
  sensorContext.captured_at = new Date().toISOString()
}

function hasAnySensorData(): boolean {
  return Boolean(sensorContext.location)
}

async function captureLocation() {
  if (!('geolocation' in navigator)) {
    message.warning('当前浏览器不支持定位传感器')
    return
  }

  sensorLoading.location = true
  sensorStatus.location = '请求中...'
  ...
  try {
    ...
    sensorStatus.location = '失败/拒绝'
    message.warning(e?.message || '定位获取失败，请检查浏览器权限')
  } finally {
    sensorLoading.location = false
  }
}
...
const handleSubmit = async () => {
  ...
  try {
    const requestData: TripFormData = {
      ...
      ...(hasAnySensorData() ? { sensor_context: JSON.parse(JSON.stringify(sensorContext)) } : {})
    }
...
@media (max-width: 768px) {
  ...
  /* 避免 iOS Safari 输入框聚焦时自动放大 */
  .home-container :deep(input),
  .home-container :deep(textarea),
  .home-container :deep(.ant-select-selector),
  .home-container :deep(.ant-picker) {
    font-size: 16px;
  }
}
